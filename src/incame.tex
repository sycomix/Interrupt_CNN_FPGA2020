...
In order to support multi-task scheduling and solve the hardware resources conflicts, interrupt is introduced to CPU \cite{jen1974processor}. In this paper, we also use the concept of interrupt to support multi-task on the accelerator.
...
To accelerate the post-processing operations of the CNN based methods, hardware modules are implemented for the CPU-intensive Softmax \cite{Softmax-wiki} and Normalization operations \cite{Norm}. Some task-related software optimizations, such as Ranking and NMS \cite{NeubeckGool-NMS}, are processed in the CPU side of the embedded MPSoC\cite{MPSoC}.
